...
{
	INT8U i;
	Sensor_Init_t devSensorInitTable[12];
	...
	for (i = 0; i < num; i++)
	{
		...
		if (devSensorInitTable[i].id == 0xc0 && devSensorInitTable[i].id_reg == 0x00 && devSensorInitTable[i].w_cmd == 0x62)
		{
			...
			data[0] = 0xfc;
			data[1] = 0x16;
			hal_iic08bitAddrWrite(0x62, data[0], &data[1], 1);
		}
#endif
...
void sensorInit(void)
{
	...
	if (0x76 == devSensorInit->id) // OV7670
	{
		if (senser_select == 0)
		{
			sensor_rotate(3);
...
void sensorChange()
{
	...
	if (senser_select)
	{
		...
		sensorInit_table();
		deg_Printf("sensorid:%d\n--------", devSensorInit->id);
		if (0xa6 == devSensorInit->id)
			sensor_iic_write((u8[]){0x4a, 0x0C});
		else
		{
	...
	else
	{
		...
		if (0xa6 == devSensorInit->id)
		{
			hal_gpioWrite(REAR_SENSOR_RESET_CH, REAR_SENSOR_RESET_PIN, GPIO_HIGH); // 100B×¢ÊÍ¸ÃÐÐ
