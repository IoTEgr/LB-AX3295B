...
void snake_init()
{
	snake *tail;
	int i;
	tail = snake_get_buf(); // 从蛇尾开始，头插法，以x,y设定开始的位置//
	tail->x = 0;
	tail->y = 0;
	tail->next = NULL;
	for (i = 0; i < 3; i++) // 初始长度为4
	{
		head = snake_get_buf();
		head->next = tail;
		head->x = tail->x + 1;
